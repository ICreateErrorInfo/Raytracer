...
namespace Raytracer
{
    class image_texture : Texture
    {
        ...
        public override Vektor value(double u, double v, Vektor p)
        {
            ...
            Vektor pixel = new Vektor(data[index] * color_scale, data[index + 1] * color_scale, data[index + 2] * color_scale);

            return new Vektor(pixel.X, pixel.Y, pixel.Z);
        }
        public Vektor GetPixel(int x, int y)
        {
            var index = y * bytes_per_scanline + x * bytes_per_pixel;
            Vektor pixel = new Vektor(data[index], data[index + 1], data[index + 2]);
